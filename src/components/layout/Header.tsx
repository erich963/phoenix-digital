      ...
      <div style={{ paddingLeft: 'min(6.5rem, 8vw)', paddingRight: 'min(6.5rem, 8vw)' }}>
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex-shrink-0">
            <Image src="/logo.png" alt="Phoenix Digital" width={180} height={36} className="h-8 w-auto" priority />
          </Link>
          ...
          <nav className="px-6 py-4 flex flex-col gap-1">
            {navLinks.map((link) => (
              <Link
                ...
                className={`px-3 py-2.5 text-sm font-medium rounded-lg transition-colors hover:text-slate-900 hover:bg-slate-50 ${
            ...
            <div className="pt-3 pb-1">
              <Link
                href="/contact"
                onClick={() => setMobileOpen(false)}
                className="flex items-center justify-center w-full px-5 py-3 text-sm font-semibold text-white bg-slate-900 hover:bg-slate-700 rounded-lg transition-colors"
              >
                Book a Consultation
              </Link>
            </div>
